...
gibbs_theta <- function(SS, iota_dist, par_prior, theta, param, Y){
      ...
      if(iota_dist == "exponential") { # Markovian process - 2 parameterizations
            ...
            if(param == "bg") { # theta = (beta, gamma)
                  ...
                  beta_new   <- stats::rgamma(1, shape = a_beta  + n_I, rate = b_beta  + integral_SI)
                  gamma_new  <- stats::rgamma(1, shape = a_gamma + n_R, rate = b_gamma + integral_I )
                  R0_new     <- S0 * beta_new / gamma_new

            } else if (param == "bR") { # theta = (beta, R0)

                  beta_new   <-     stats::rgamma(1, shape = a_beta + n_R + n_I, rate = b_beta + integral_SI + integral_I * S0 / R0_curr)
                  R0_new     <- 1 / stats::rgamma(1, shape = a_R0   + n_R      , rate = b_R0   + beta_new * S0 * integral_I             )
            ...
            lambda_new <- stats::rgamma(1, shape = a_lambda + n_R, rate = b_lambda + sum_iota)
